...
var querySql = async function (sql, params) {
  // 写sql语句自由查询
...
var add = async function (addObj, tableName) {
  // 添加数据
  if (!addObj) {
    return;
  }
...
var addIfNotExist = async function (addObj, whereObj, tableName) {
  // 添加数据
  if (!addObj) {
    return;
  }
  if (!whereObj) {
    return add(addObj, tableName);
  }
  await mssql.close(); // close
  var connection = await getConnection();
  var request = connection.request();
...
var addList = async function (addObjs, tableName) {
  // 添加数据
...
var addListIfNotExist = async function (addObjs, uniqueFieldName, tableName) {
  // 添加数据
  if (!addObjs || addObjs.length === 0) {
    return;
  }
...
var update = async function (updateObj, whereObj, tableName) {
  // 更新数据
  await mssql.close(); // close
  var connection = await getConnection();
  var request = connection.request();
